...
def get_three_phase_transformer_data(network:GraphModel) -> dict:
    ...
    if cim.PowerTransformer in network.graph:
        for transformer in network.graph[cim.PowerTransformer].values():
            if not transformer.TransformerTanks:
                ptf_data = get_data(transformer, data_profile.PowerTransformer)
                ptf_data['PowerTransformerEnd'] = []
                for end in transformer.PowerTransformerEnd:
                    ...
                    ptf_data['PowerTransformerEnd'].append(end_data)

                transformer_data[transformer.mRID] = ptf_data
