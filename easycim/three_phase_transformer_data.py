...
def get_three_phase_transformer_data(network: GraphModel) -> dict:
    ...
    if cim.PowerTransformer in network.graph:
        for transformer in network.graph[cim.PowerTransformer].values():
            ...
            if three_phase:
                xfmr_data = get_data(transformer,
                                     data_profile.PowerTransformer)
                xfmr_data['PowerTransformerEnd'] = []
                for end in transformer.PowerTransformerEnd:
                    end_data = {}
                    end_data['Terminal'] = get_data(end.Terminal,
                                                    data_profile.Terminal)
                    end_data['Terminal'][
                        'ConnectivityNode'] = end.Terminal.ConnectivityNode.name
                    end_data = get_data(end, data_profile.TransformerEnd,
                                        end_data)
                    end_data = get_data(end, data_profile.PowerTransformerEnd,
                                        end_data)
                    # Star impedance
                    if end.StarImpedance is not None:
                        data = get_data(end.StarImpedance,
                                        data_profile.TransformerStarImpedance)
                    ...
                    for impedance in end.ToMeshImpedance:
                        data = get_data(impedance,
                                        data_profile.TransformerMeshImpedance)
                    ...
                    if end.CoreAdmittance is not None:
                        data = get_data(end.CoreAdmittance,
                                        data_profile.TransformerCoreAdmittance)
