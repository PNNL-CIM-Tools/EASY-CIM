...
def get_regulator_data(network: GraphModel) -> dict:
    """This method returns a dictionary of single-phase and three-phase capacitor
    ...
    network.get_all_edges(cim.RegulatingControlModeKind)
  

    regulator_data = {}
    if cim.TapChanger in network.graph:
        for regulator_equip in network.graph[cim.TapChanger].values():
            regulator_data[regulator_equip.mRID] = get_data(regulator_equip, data_profile.TapChanger)
    
    if cim.RatioTapChanger in network.graph:
        for regulator_equip in network.graph[cim.RatioTapChanger].values():
            regulator_data[regulator_equip.mRID] = get_data(regulator_equip, data_profile.RatioTapChanger)
    
    if cim.TapChangerControl in network.graph:
        for regulator_equip in network.graph[cim.TapChangerControl].values():
            regulator_data[regulator_equip.mRID] = get_data(regulator_equip, data_profile.TapChangerControl)
